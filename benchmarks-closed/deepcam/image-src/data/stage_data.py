...
def load_batch_parallel(executor, files, comm_size, comm_rank):
...
def stage_instance_data(stage_comm, instance_comm, instance_node_comm,
    ...
    if not prepare_only:
        ...
        if full_dataset_per_node:
            ...
            files_shard = get_shard(files, isize // lsize, isize % lsize)
        
        # create tags
        tag = os.path.basename(files_shard[0]).split("-")[0]
    ...
    # submit first batch
    #fdata_handle = executor.submit(load_batch, files_shard[0:min(num_files_per_shard, batch_size)],
    ...
    for bid in range(1, num_batches+1):
        ...
        if bid < num_batches:
            ...
            #fdata_handle = executor.submit(load_batch, files_shard[batch_start:batch_end],
            #                               ssize if stage_mode == "global" else 1,
            #                               srank if stage_mode == "global" else 0)
...
def stage_data_helper(global_comm, num_instances, instance_id, instance_comm,
    ...
    for stage_filter in stage_filter_list:
        ...
        if grank == 0:
            files = sorted(glob(os.path.join(stage_source_directory, os.path.basename(stage_filter))))
        else:
            files = None
        ...
        files = global_comm.bcast(files, 0)
        ...
        file_stats[stage_filter] = len(files) 
        ...
        if verify:
            ...
            num_files = 0
            if local_rank == 0:
                num_files = len(glob(os.path.join(stage_target_directory, os.path.basename(stage_filter))))

            # if every node hosts a shard, we need to sum the results, if not we need to make sure everybody has the same
            num_files = instance_comm.allreduce(num_files)
            
            if full_dataset_per_node:
                num_files = int(num_files / float(instance_node_comm.Get_size()))
