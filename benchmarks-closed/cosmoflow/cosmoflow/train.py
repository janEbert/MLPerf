...
class LRScheduler():
    ...
    def __call__(self, iteration: int) -> float:
        current_epoch = iteration // self.steps_per_epoch
        ...
        if current_epoch != self.last_epoch:
            print(f"Learning rate for epoch {current_epoch} is set to {lr}")
...
def main(args: argparse.Namespace):
    ...
    #loss_fn.hybridize(static_shape=True,
    #                  static_alloc=True)
...
def initialize_model(dist_desc: utils.DistributedEnvDesc, 
    ...
    network = model.CosmoflowWithLoss(conv_kernel=3, layout=data_layout, 
                                      use_wd=use_wd, dropout_rate=dropout)
    network.init(mx.gpu(dist_desc.local_rank), batch_size, use_wd,
                 dist_desc, checkpoint)
...
def fit_epoch(model: mx.gluon.HybridBlock, trainer: mx.gluon.Trainer, 
    ...
    def _step(idx, data, label, perf_counter, prefetch_iter = None):
        with utils.ProfilerSection(f"iter_{idx}", profile=profile):
            ...
            perf_counter.update_processed(batch_size * dist_desc.size)
